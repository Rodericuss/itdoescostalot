defmodule IdcalWeb.IncomeLive.Index do
  ...
  def handle_event("delete_category", %{"id" => id}, socket) do
    ...
    case Finances.delete_income_category(category) do
      {:ok, _} ->
        {:noreply,
         socket
      ...
      {:error, _changeset} ->
        {:noreply, put_flash(socket, :error, gettext("Cannot delete category with sources."))}
    end
  end
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="flex items-center justify-between">
        <div>
          ...
          <h1 class="font-cinzel font-bold text-3xl text-gold mt-1">{gettext("Income")}</h1>
        </div>
        <button phx-click="new_category" class="btn-medieval">
          <.icon name="hero-plus" class="size-5" /> {gettext("New Category")}
        </button>
      </div>
      ...
      <div :for={category <- @categories} class="panel p-5 space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="panel-title text-xl">{category.name}</h2>
          <div class="flex gap-2">
            <button phx-click="new_source" phx-value-category-id={category.id} class="btn-medieval text-sm">
              <.icon name="hero-plus" class="size-4" /> {gettext("Add Source")}
            </button>
        ...
        <p :if={category.sources == []} class="ml-4 text-muted italic text-sm">
